/* ═══════════════════════════════════════════════════════════════════════════════
   نجوم المدرسة — تنبيهات الحصص  (window.SIJIL_NOTIFY)
...
(function () {
  ...
  var IOS_STEPS = [
    ...
    "أغلق سفاري وافتح «نجوم المدرسة» من أيقونته الجديدة",
  ...
  function state() {
    ...
    if (iosInst) msg = "لتصلك التنبيهات على الآيفون ثبّت «نجوم المدرسة» على شاشتك الرئيسية أولاً — أبل لا تسمح بها من سفاري";
    else if (!sup) msg = "هذا المتصفح لا يدعم التنبيهات — جرّب متصفح جوالك أو ثبّت «نجوم المدرسة» على الشاشة الرئيسية";
    ...
    else if (!pushOn) msg = "التنبيهات مفعّلة — سيصلك تذكير قبل كل حصة بـ " + minsAr(leadMin()) + " ما دام «نجوم المدرسة» مفتوحاً على الجهاز";
    else msg = "التنبيهات مفعّلة — سيصلك تذكير قبل كل حصة بـ " + minsAr(leadMin()) + " ما دام «نجوم المدرسة» مفتوحاً؛ وإن كان مغلقاً فقد يصل قبلها بقليل";
    return {
  ...
  function enable() {
    ...
    return Promise.resolve().then(function () { return Notification.requestPermission(); }).then(function (p) {
      ...
      return swReg().then(function () {
        ...
        setOn(false); paint();
        return { ok: false, code: "sw", message: "تعذّر تجهيز خدمة التنبيهات في هذا المتصفح — افتح «نجوم المدرسة» من الشاشة الرئيسية بعد تثبيته، أو جرّب متصفحاً آخر", error: String(e && e.message || e) };
      });
    }).catch(function (e) {
      setOn(false); paint();
