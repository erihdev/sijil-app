/* ═══════════════════════════════════════════════════════════════════════════════════════
   نجوم المدرسة — بوابة الطالب: محتوى التبويبات (موادي · مهامي · دروسي · شهادتي) + التدرّب والشارات
   ───────────────────────────────────────────────────────────────────────────────────────
...
(function () {
  ...
  function nCardHtml() {
    ...
    if (nIosNeedsInstall()) {
      return '<div class="card"><b>🔔 نبّهني بالجديد</b>'
        ...
        + '<li>افتح «نجوم المدرسة» من الأيقونة الجديدة</li><li>ثم اضغط «نبّهني بالجديد» هنا</li></ol></div>';
    }
    if (nState() === "denied") {
      return '<div class="card"><b>🔔 التنبيهات موقوفة</b><p class="sub" style="margin:6px 0 0">أنت (أو والدك) منعتَ التنبيهات لهذا الموقع. افتح إعدادات المتصفح ← الإشعارات ← اسمح لـ«نجوم المدرسة».</p></div>';
    }
    if (nOn()) {
  ...
  function nBind(el) {
    ...
    if (on) on.onclick = function () {
      ...
      Promise.resolve(ask).then(function (p) {
        ...
        nSet(true);
        nShow("نجوم المدرسة — تم ✅", "بنبّهك أول ما يرسل معلمك شيئاً جديداً.", "sijil-s-on");
        nSubscribe();
        try { ST.refresh(); } catch (e) { }
      });
    };
    if (off) off.onclick = function () { nSet(false); nUnsubscribe(); try { ST.refresh(); } catch (e) { } };
    if (tst) tst.onclick = function () { nShow("نجوم المدرسة — تنبيه تجريبي 🔔", "كذا بيوصلك التنبيه إن شاء الله.", "sijil-s-test"); };
  }
  ...
  function parentText() {
    ...
    L.push("");
    L.push("يُعرض هذا الملخّص من بوابة الطالب في تطبيق «نجوم المدرسة»، والدرجات من البنود المرصودة حتى تاريخه.");
    return L.join("\n");
  }
  ...
  function pToday(dt) {
    ...
    /* الرصد في «نجوم المدرسة» يومي لكل معلم لا لكل حصة (مستند recs/{tid}_{cid} مفتاحه التاريخ)،
